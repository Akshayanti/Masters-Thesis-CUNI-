...
def find_with_more_than(filename, cutoff):
	""""Given the filename, and the cutoff value, find instances that have been annotated with confidence score >= cutoff value,
	and write them out in a file"""
	out_lines = []
...
def generate_plots(files, confidence):
	"""Generate a Density Rug Plot for True Positive Labelled Data, with confidence of prediction less than specified confidence value."""
	a = {"O": [], "S-verb": [], "S-aux": []}
	filename = files[0]
	with open(filename, "r", encoding="utf-8") as infile:
		for line in infile:
...
if __name__ == "__main__":
	...
	group1.add_argument('--above', action='store_true', help="Filter values with prediction confidence score ABOVE \'--confidence\' value")
	group1.add_argument('--below', action='store_true', help="Filter values with prediction confidence score BELOW \'--confidence\' value")
	group1.add_argument('--plot', action='store_true', help="Do not filter values, but generate rug-plot of distribution of confidence scores, with maximum cutoff value as specified in \'--confidence\'"
	                                                        "argument")
	parser.add_argument('-c', '--confidence', type=float, help="Float value of confidence score to compare against.", required=True)
