	...
	for filename1 in fiction news nonfiction social spoken_conversational; do
		for filename2 in fiction news nonfiction social spoken_conversational; do
			if ! [ $filename1 = $filename2 ]; then
				...
				python3 klcpos3.py --single_source --source `echo $filename1`_400.conllu --target `echo $filename2`_400.conllu | cut -f2 >> klcpos3.tsv;
				python3 klcpos3.py --single_source --target `echo $filename1`_400.conllu --source `echo $filename2`_400.conllu | cut -f2 >> klcpos3.tsv;
				echo "" >> klcpos3.tsv;
			fi;
		done;
		echo $filename1 >> politeness_`echo $seedval`;
		python3 get_formality_scores.py `echo $filename1`_400.conllu >> politeness_`echo $seedval`;
...
python3 get_scores_with_sd.py 2 politeness_* > pl/formality;
rm -f klcpos3_scores* politeness_*;
...
for seedval in `seq 1 100`; do
	...
	for filename in fiction news nonfiction social spoken_conversational; do
		...
		echo $filename >> politeness_self_`echo $seedval`;
		python3 get_formality_scores.py `echo $filename`_400.conllu >> politeness_self_`echo $seedval`;
		echo "" >> politeness_self_`echo $seedval`;
	done;
...
python3 get_scores_with_sd.py 2 politeness_self* > pl/formality_self;
rm -f *.py *.conllu klcpos3_self_* politeness_self*;
